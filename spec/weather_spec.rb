...
describe Weather do
  describe "#random_weather" do
    it 'should return a random weather state, stormy if true, sunny if false' do
      weather = Weather.new
  ...
  describe "#weather_status" do
    it 'should return the weather status (sunny or stormy) as a string' do
      weather = Weather.new
      expect(subject.weather_status).to eq "stormy" if weather.weather_stormy?.eql? false
    end
  end

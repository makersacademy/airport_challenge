...
describe Airport do
  ...
  let (:badweather) {double :badweather, check: false}
  let (:goodweather) {double :goodweather, check: true}
  ...
  context "Bad weather routines" do
    ...
    it "does not allow planes to land during storms" do
      expect {airport.land(airport, badweather)}.to raise_error("Can't land until skies are clear")
    end

    it "does not allow planes to take off during storms" do
      expect {airport.takeoff(airport, badweather)}.to raise_error("Can't get going until skies are clear")
    end
  ...
  context "Tarmac" do
    ...
    it "knows which planes are visiting" do
      airport.land(airplane, goodweather)

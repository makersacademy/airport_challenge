...
describe Airport do
let(:plane) {double(:plane,:flying= => true, :flying? => true)}
  ...
  describe '::new' do
    ...
    allow(airport).to receive(:is_stormy?).exactly(2).times.and_return(false)
  ...
  describe '#instruct_plane_to_take_off' do
    ...
    it 'should not let a plane that isn\'t at the airport take off' do
      allow(subject).to receive(:is_stormy?).exactly(2).times.and_return(false)
      subject.instruct_plane_to_land(plane)
      plane2 = double(plane)
      expect{subject.instruct_plane_to_take_off(plane2)}.to raise_error ("Cannot take off - this plane is not at the airport")
    end
  end
end
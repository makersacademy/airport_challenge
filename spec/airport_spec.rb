...
let(:plane) {double :plane}
  ...
  it "can  get a plane to landing" do
  ...
  expect(subject.confirms_landed(plane)).to eq("The plane #{@plane_number} has landed")
  end
  ...
  expect(subject.confirms_taken_off(plane)).to eq("The plane #{@plane_number} has taken off")
  end

  it "doesn't let a plane to land if it is full" do
  allow(subject).to receive(:stormy) {false}
  55.times{subject.landed_planes(plane)}
  expect(subject.landed_planes(plane)).to eq("The airport is full, the plane #{@plane_number} can't land")
  end

  it "forbids planes to land if the weather is stormy" do
  allow(subject).to receive(:stormy) {true}
  expect(subject.landed_planes(plane)).to eq("The plane #{@plane_number} can't land due to stormy weather")
  end

  it "forbids planes to take off if the weather is stormy" do
  allow(subject).to receive(:stormy) {true}
  expect(subject.taken_off_planes(plane)).to eq("The plane #{@plane_number} can't take off due to stormy weather")
  end

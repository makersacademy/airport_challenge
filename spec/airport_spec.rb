...
describe Airport do
  ...
  context "raises an error if the airport is full" do
    it "returns error when airport is at max capacity and another is added" do
      ...
      expect { airport.land(Plane.new) }.to raise_error(RuntimeError, /Airport is full, plane cannot be landed/)
  ...
  context "prevent takeoff when stormy" do
    it "raises an error message when #take_off is executed and #weather_report is stormy" do
      ...
      expect { subject.take_off(plane) }.to raise_error "Weather is stormy, plane not permitted to take off."
    end
  end
end

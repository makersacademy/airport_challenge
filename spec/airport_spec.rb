...
describe Airport do
  ...
  describe 'capacity' do
    ...
    it 'can only accept an integer' do
      expect { described_class.new(double :fake_number) }.to\
        raise_error 'Not an integer'
    end

    it 'only accepts a positive integer' do
      random_capacity = -Random.rand(200)
      expect { described_class.new random_capacity }.to\
        raise_error 'Not positive'
    end
  end
  ...
  describe '#take_offs' do
    ...
    it 'raises an error when too many planes try to take off' do
      ...
      expect { subject.take_offs(4) }.to\
        raise_error 'Too many planes taking off'
    end
  ...
  describe '#landings' do
    ...
    it 'only allows actual planes to land' do
      expect { subject.landings(fake_plane, fake_plane, fake_plane) }.to\
        raise_error "Not planes"
    end
  end

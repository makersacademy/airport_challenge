...
# test land
  ...
  it "will not allow the plane to land when it's stormy" do
    ...
    allow(airport.weather_forecaster).to receive(:stormy?) { true }

    expect { airport.land(plane) }.to raise_error("Sorry, it's too stormy to land")
    expect(airport.planes).to eq []
  end

  it "will not allow the plane to land if it's landed already" do
    landed_plane = plane
    airport.planes = [landed_plane]

    expect { airport.land(plane) }.to raise_error("Sorry, the plane is landed already")
  end

  it "will allow the plane to land when it's sunny" do
    airport.planes = []
    allow(airport).to receive(:full?) { false }
    allow(airport.weather_forecaster).to receive(:stormy?) { false }
...
# test takeoff
  ...
  it "will not allow the flying plane to take off when it's flying" do
    airport.planes = []

    expect { airport.takeoff(plane) }.to raise_error("Sorry, the plane is flying already")
  end

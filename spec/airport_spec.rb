...
describe Airport do
  ...
  describe '#clear_plane_for_take_off' do
    it 'clears planes to take off' do
      ...
      expect{airport.clear_plane_for_take_off}.to change{subject.planes.length}.by(-1)
    end

    it 'does not clear planes for take off if the weather is stormy' do
      allow(airport).to receive(:check_weather) {true}
      expect{airport.clear_plane_for_take_off}.to raise_error 'Permission denied, the weather is too dangerous.'
    end
  end

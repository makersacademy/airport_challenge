...
describe Airport do
  ...
  context 'When weather is fine' do
    before do
      allow(subject).to receive(:too_stormy) { false }
    end
  end
  describe '#land' do
    it 'lands a plane' do
    ...
    it 'raises an error if landing a plane when the airport is full' do
  ...
  context 'When weather is stormy' do
    before do
      allow(weather).to receive(:too_stormy?).and_return true # For some reason this is not setting the too_stormy too eq true as I would expect it too. When too_stormy is hard coded to be true, the test passes.
    ...
    describe '#land' do
      it 'Gives Error message when too stormy too land' do
        plane = Plane.new

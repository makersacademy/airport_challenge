...
describe Airport do
  ...
  # not possible for a plane to land if it's not in the air (or in transit)
    it 'should raise an error "Plane not airborne" if trying to land when plane is not in the air (could be already in airport or in another airport)' do
  ...
  it 'should raise an error "Weather is stormy" if trying to land when airport weather is stormy' do
    allow_any_instance_of(Airport).to receive(:weather_now).and_return("stormy")
  ...
  it "(when a plane is told to land) should add plane to the planes at the airport if there's enough space and weather permits (i.e. is clear)" do
    allow_any_instance_of(Airport).to receive(:weather_now).and_return("clear")
    new_airport = Airport.new("AIRPORT")
    new_plane = Plane.new("PLANE")
  ...
  it "(when a plane is told to take off) should remove that plane from the airport inventory as long as it was there in the first place and the weather is clear" do
    allow_any_instance_of(Airport).to receive(:weather_now).and_return("clear")
    new_airport = Airport.new("AIRPORT")
    new_plane = Plane.new("PLANE")
    new_airport.airport_attributes[:planes] << new_plane

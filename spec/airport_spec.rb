...
describe Airport do
  ...
  it 'raises an error if a plane tries to land or take off when the weather is stormy' do
    ...
    expect{subject.cleared_for_landing(Plane.new)}.to raise_error("It's too stormy to land.")
    expect{subject.cleared_for_takeoff(Plane.new)}.to raise_error("It's too stormy to take off.")
  end

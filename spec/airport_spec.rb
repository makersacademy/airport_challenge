...
describe Airport do
  it 'instructs a plane to land at the airport' do
    ...
    gatwick = Airport.new(10, weather)
    expect(gatwick.land_plane(Airplane.new)).to be_a_kind_of(Array)
  end

  it 'expects everything landed at the airport to be a Plane' do
    weather = double('weather', :stormy => false)
    gatwick = Airport.new(20, weather)
    7.times { gatwick.land_plane(Airplane.new) }
  ...
  it 'expects a plane to take off from an airport and return that it has taken off' do
    weather = double('weather', :stormy => false)
    gatwick = Airport.new(20, weather)
    expect(gatwick.take_off).to eq 'airplane has taken off and is no longer in the airport'
  end
  ...
  it 'tests that if we have 6 planes, 4 fly off, then we have 2 left at the airport' do
    ...
    4.times { gatwick.take_off }
    expect(gatwick.planes.length).to eq 2
  end

  it 'raises an error preventing planes from taking off if stormy' do
    weather = double('something', :stormy => true)
    gatwick = Airport.new(20, weather)
    expect { gatwick.take_off }.to raise_error('Weather is stormy: no planes are to take off')
  end

  it 'raises an error preventing planes from landing if stormy' do
    weather = double('something', :stormy => true)
    gatwick = Airport.new(20, weather)
    expect { gatwick.land_plane(Airplane.new) }.to raise_error('Weather is stormy: no planes are to land')
  end

  it 'raises an error when the airport is full' do
    weather = double('weather', :stormy => false)
    gatwick = Airport.new(20, weather)
    expect { 21.times { gatwick.land_plane(Airplane.new) } }.to raise_error('The airport is full')
  end
  ...
  it 'raises an error if you try to land a plane when it is already in the airport' do
    ...
    gatwick = Airport.new(20, weather)
    boeing = Airplane.new
    gatwick.land_plane(boeing)
    expect { gatwick.land_plane(boeing) }.to raise_error('This airplane is already in the airport')
  end
end

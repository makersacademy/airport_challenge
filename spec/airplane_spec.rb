...
describe Airplane do
  ...
  it "Shows that by default the  current_status method will return grounded" do
    expect(subject.current_status).to eq "airborne"
  end
  it "Shows that the  current_status method will return grounded if the land method is called and airborne when the take_off method is called" do
    subject.take_off
    expect(subject.current_status).to eq "airborne"
    subject.land
    expect(subject.current_status).to eq "grounded"
  end

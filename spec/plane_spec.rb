...
describe Plane do
  ...
  it 'can take off if currently at an airport' do
    plane = Plane.new
    expect { plane.take_off }.to raise_error('This plane is not on the ground')
  ...
  describe 'stormy weather' do
    it 'will not take off if the weather is stormy' do
      ...
      expect { subject.take_off }.to raise_error('A storm prevents the plane from taking off')
    end

    it 'will not land if the weather is stormy' do
      storm_port = instance_double('Airport', :stormy? => true, :full? => false, :accept => nil)
      expect { subject.land(storm_port) }.to raise_error('A storm prevents the plane from landing')
    end
  end

  it 'will not land if the airport is full' do
    airport = instance_double('Airport', :stormy? => false, :full? => true, :accept => nil)
    expect { subject.land(airport) }.to raise_error('This airport is full')
  end
end

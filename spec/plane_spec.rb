...
describe Plane do
  let(:airport) { double(:airport, :stormy? => false, :outbound => nil) }
  let(:destination) { double(:airport, :stormy? => false) }
  subject(:docked_plane) { described_class.new(airport) }
  let(:in_flight_plane) { described_class.new(airport) }
  ...
  describe '#land_at' do
    ...
    context 'when the plane is not in flight' do
      it 'raises an exception' do
        expect do
          docked_plane.land_at destination
    ...
    context 'when the plane is in flight' do
      it 'lands at an airport and confirms' do
        expect(airport).to receive(:outbound) { in_flight_plane }
        in_flight_plane.take_off
        expect(destination).to receive(:inbound) { in_flight_plane }
        expect(in_flight_plane).to receive(:new_location) { destination }
      ...
      it 'raises exception and does not land if destination weather is stormy' do
        ...
        in_flight_plane.take_off
        expect do
          in_flight_plane.land_at destination
  ...
  describe '#location' do
    context 'when plane is stationed at an airport' do
      it 'returns the airport code' do
        ...
        expect(docked_plane.location).to eq "#{airport}"
      end
    end
    ...
    context 'when a plane is in flight' do
      it "returns \'In flight\'" do
        docked_plane.take_off
        expect(docked_plane.location).to eq "In flight"
      end
    end
  end

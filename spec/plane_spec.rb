...
describe Plane do
  ...
  describe '#landed?'do
    it ' responds "true"(weather = sunny, airport != full) after receiving the "land" method'do
    ...
    it 'responds "false "(weather = stormy, airport != full) after receiving the "land" method'do
      allow(airport).to receive(:full?).and_return(false)
      allow(airport).to receive(:planes).and_return([])
    ...
    it 'responds "false" (weather = sunny, airport = full) after receiving the "land" method'do
    ...
    it ' reponds "false"(weather = sunny) after receiving the "takeoff" method'do
      allow(airport).to receive(:planes).and_return([subject])
      subject.takeoff(airport, "sunny")
      expect(subject.landed?).to eq false
    end

    it ' responds "true" (weather = stormy) after receiving the "takeoff" method'do
      allow(airport).to receive(:planes).and_return([subject])

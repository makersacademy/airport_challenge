...
describe Plane do
  ...
  it 'Can receive the "land" command with 2 arguments.' do
    airport = double(:landed_planes => [], :capacity => 30, :landed_planes_history => [])
    plane = Plane.new(airport)
    airport.landed_planes_history << plane
...
context 'During stormy weather' do
  it 'Will not be permitted to take-off.' do
    ...
    airport = double(:Airport, :confirm_plane_landed => true, :landed_planes => [], :capacity => 30, :landed_planes_history => [])
    plane = Plane.new(airport)
  ...
  it 'Will not be permitted to land.' do
    ...
    airport = double(:confirm_plane_landed => true, :landed_planes => [], :capacity => 30, :landed_planes_history => [])
    plane = Plane.new(airport)
...
context 'When the airport is full' do
  ...
  it 'Flying plane will not be permitted to land at this airport.' do
    weather = double(:stormy? => false)
    default_capacity = 30
    airport = double(:capacity => default_capacity, :landed_planes => [], :confirm_plane_landed => true, :landed_planes_history => [])
    plane = Plane.new(airport)

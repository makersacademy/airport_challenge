...
class Airport
  ...
  def initialize(weather: Weather.new) # Need
    @weather = weather
    @planes_in_airport = []
  ...
  def land(plane)
    fail "This plane is already landed." unless plane.airborne
  ...
  def take_off(plane)
    ...
    fail "Plane is not in this airport" unless planes_in_airport.include? plane

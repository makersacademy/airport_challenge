...
class Airport
  ...
  def move_to_hangar(plane)
    fail "Only newly made planes can be moved to Hangar" if plane.location != 'factory'
  ...
  def take_off(plane)
    check_weather
    fail "Plane not at this Airport" if plane.location != airport_name
  ...
  def land(plane)
    check_weather
    fail "Plane is not flying" if plane.location != 'in transit'
    fail "Bad weather" if stormy
    fail "The hangar is full" if full?
    plane.update_location(airport_name)
    plane.lands

...
class Airport
  ...
  def initialize(identifier = "DEF", capacity = DEF_CAPACITY)
    raise error "Airport identifier must be a string" unless identifier.is_a?(String)
    raise error "Airport identifier must be three capital letters" unless identifier.length == 3 && identifier.upcase == identifier
    @identifier = identifier.to_s.to_sym
    raise error "Airport capacity must be an integer" unless capacity.is_a?(Integer)
    raise error "Airport capacity must be positive integer" unless capacity > 0
    @capacity = capacity
    @planes_parked = []

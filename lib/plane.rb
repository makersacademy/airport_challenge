class Plane
  ...
  def land(airport)
    ...
    raise "You cannot land in stormy weather" if airport.stormy_weather
    @in_flight = false
  ...
  def take_off(airport = @current_airport)
    raise "You are already in flight" if @in_flight
    raise "You cannot take off in stormy weather" if airport.stormy_weather
